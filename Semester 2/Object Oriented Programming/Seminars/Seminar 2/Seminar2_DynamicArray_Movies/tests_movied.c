...
void testSetGetElemPosArray() {
	...
	assert(equalMovieD(&element,&m1));

	element = getElem(&arr, 1);
	assert(equalMovieD(&element, &m2));

	element = getElem(&arr, 2);
	assert(equalMovieD(&element, &m3));


	
	MovieD m4 = createMovieD("Movie4", "Director4", 1997, 3.01);
	TElem replacedElement = setElem(&arr,m4, 0);
	assert(equalMovieD(&replacedElement, &m1));
	element = getElem(&arr, 0);
	assert(equalMovieD(&element, &m4));

	destroyMovieD(&replacedElement);
	destroyArray(&arr);
