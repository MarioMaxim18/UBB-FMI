...
void testAddArrayMovieS()
{
	...
	MovieS* elem1 = (MovieS*) getElem(&arr, 0);
	...
	MovieS* lastElement = getElem(&arr, arr.length-1);
...
void testSetGetElemPosArrayMovieS() {
	...
	MovieS* replacedElement = setElem(&arr, m4, 0);
...
void testRemoveFromArrayMovieS(){
	...
	assert(size(&arr) == 5);
	MovieS* deletedElement = removeLast(&arr);
	assert(size(&arr) == 4);
	assert(!search(&arr, deletedElement));
	assert(deletedElement->year == 2022);
	destroyMovieS(deletedElement);

	MovieS* m0 = getElem(&arr, 0);
	assert(m0->year == 1999);
	MovieS* m1 = getElem(&arr, 1);
	assert(m1->year == 2021);
	MovieS* m2 = getElem(&arr, 2);
	assert(m2->year == 1976);
	MovieS* m3 = getElem(&arr, 3);
	assert(m3->year == 2000);
	...
	//Notice we don't put the & anymore
	destroyMovieS(deletedElement);
